...
namespace InventoryManagementSystem.Repository
{
    ...
    {
        if (CheckSupplierNameExists(name))
        {
    ...
    public Supplier FindSupplierById(int id)
    {
        try
        {
            ...
            var findSupplier = _context.Suppliers.Where(item => item.SupplierId == id).FirstOrDefault();
    ...
    public bool CheckSupplierNameExists(string name)
    {
        return _context.Suppliers.Any(item => item.SupplierName == name);
    }
    ...
    public void UpdateSupplier(int id, int fieldChoice, string newValue)
    {
        ...
        {
            case 1:
                if (CheckSupplierNameExists(newValue))
                {
                    throw new DuplicateItemException("Supplier name already exists. Please use a different name.");
                }
    ...
    public Supplier GetSupplierDetails(int id)
    {
        ...
        return _context.Suppliers.FirstOrDefault(item => item.SupplierId == id);
